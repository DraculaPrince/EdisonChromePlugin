...
// 	process.env.REACT_APP_BASE_URL && process.env.REACT_APP_ENV !== "dev"
...
http.interceptors.response.use(
	(response: any) => {
		...
		if (
			response &&
			response.data &&
			[...TOKEN_CODE_ERROR_LIST].indexOf(response.data.ret) > -1
		) {
			if (
			...
			(response && response.data && response.data.ret !== 200)
		) {
			errorToast({ msg: `${apiName || ""}${response.data.msg}` });
			removePending(response.config);
			return Promise.reject(response.data);
...
// axios跨域方法
axios.jsonp = (url: any): Promise<any> =>
	new Promise((resolve: any) => {
		(window as any).jsonCallBack = (result: any) => {
			resolve(result);
		};
